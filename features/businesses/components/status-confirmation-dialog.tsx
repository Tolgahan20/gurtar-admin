...
const STATUS_REASONS = {
  activate: [
    { value: "requirements_met", label: "All Requirements Met" },
    { value: "issues_resolved", label: "Previous Issues Resolved" },
    { value: "verification_complete", label: "Verification Process Complete" },
    { value: "appeal_approved", label: "Appeal Approved" },
  ],
  deactivate: [
    { value: "policy_violation", label: "Policy Violation" },
    { value: "customer_complaints", label: "Multiple Customer Complaints" },
    { value: "quality_concerns", label: "Quality Control Issues" },
    { value: "temporary_suspension", label: "Temporary Suspension Required" },
  ],
} as const
...
export function StatusConfirmationDialog({
  ...
  const title = business.is_active ? "Deactivate Business" : "Activate Business"
  const description = business.is_active
    ? `Are you sure you want to deactivate ${business.name}? This will prevent them from operating on the platform.`
    : `Are you sure you want to activate ${business.name}? This will allow them to operate on the platform.`
  const actionText = business.is_active ? "Yes, Deactivate" : "Yes, Activate"
  const reasonsList = business.is_active ? STATUS_REASONS.deactivate : STATUS_REASONS.activate
      ...
      <AlertDialogContent className="sm:max-w-[425px]">
          ...
          {reasonType === "predefined" ? (
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder={`Enter detailed reason for ${business.is_active ? 'deactivating' : 'activating'} (minimum 10 characters)`}
                className="h-20"
              />
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            className={cn(
              business.is_active ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700",
